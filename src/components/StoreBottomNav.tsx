  ...
  return (
    <nav className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 z-[60] pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.04)]">
      <div className="max-w-md mx-auto flex items-center justify-around h-16 px-6">
        <Link href={`/${username}`} className={`flex flex-col items-center justify-center w-16 h-full gap-1 transition-colors ${isShop ? 'text-[#111111]' : 'text-slate-400 hover:text-slate-900'}`}>
          <div className="relative">
            {isShop && <div className="absolute inset-0 bg-slate-200 rounded-full blur-md opacity-50 scale-150"></div>}
            {isShop ? (
              <div className="w-5 h-5 bg-[#111111] rounded-md rotate-45 relative flex items-center justify-center shadow-inner">
        ...
        <Link href={`/${username}/saved`} className={`flex flex-col items-center justify-center w-16 h-full gap-1 transition-colors ${isSaved ? 'text-[#111111]' : 'text-slate-400 hover:text-slate-900'}`}>
          {isSaved ? <Heart className="w-5 h-5 fill-current" /> : <Heart className="w-5 h-5" />}
          <span className="text-[10px] font-extrabold">Saved</span>
        </Link>
        
        <Link href={`/${username}/track`} className={`flex flex-col items-center justify-center w-16 h-full gap-1 transition-colors ${isTrack ? 'text-[#111111]' : 'text-slate-400 hover:text-slate-900'}`}>
