...
export default function RequestForm({ params }: { params: { username: string, productSlug: string } }) {
  ...
  if (!product || !business) {
    return <div className="min-h-screen bg-slate-50 flex items-center justify-center"><div className="w-8 h-8 border-4 border-pink-200 border-t-pink-500 rounded-full animate-spin"></div></div>;
  }
        ...
        <div className="max-w-md mx-auto w-full flex-1 flex flex-col">
          ...
          <div className="mt-8 space-y-3 pb-8">
            <Link href={`/${business.username}/track`} className="flex">
              <button className="w-full h-14 rounded-2xl bg-pink-500 hover:bg-pink-600 text-white font-extrabold text-lg flex items-center justify-center gap-2 shadow-lg shadow-pink-500/30 transition-all">
                Track Request
              </button>
            </Link>
      ...
      <div className="max-w-md mx-auto">
        ...
        <form onSubmit={handleSubmit} className="px-4 mt-6">
          ...
          <div className="space-y-4">
            <div>
              <label className="text-xs font-bold text-slate-700 mb-1.5 block">Full Name <span className="text-pink-500">*</span></label>
              <div className="relative">
                <User className="absolute left-4 top-1/2 -translate-y-1/2 w-4 h-4 text-slate-400" />
                <input required type="text" value={formData.name} onChange={(e) => setFormData({...formData, name: e.target.value})} placeholder="Enter your full name" className="w-full h-14 pl-11 pr-4 rounded-xl border border-slate-200 bg-white outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 text-sm font-medium transition-all" />
              </div>
            </div>
            
            <div>
              <label className="text-xs font-bold text-slate-700 mb-1.5 block">Phone Number <span className="text-pink-500">*</span></label>
              <div className="relative flex">
                ...
                <input required type="tel" value={formData.phone} onChange={(e) => setFormData({...formData, phone: e.target.value})} placeholder="Enter mobile number" className="w-full h-14 pl-4 pr-4 rounded-r-xl border border-slate-200 bg-white outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 text-sm font-medium transition-all" />
              </div>
            </div>
            ...
            <div>
              ...
              <div className="relative">
                ...
                <input type="email" value={formData.email} onChange={(e) => setFormData({...formData, email: e.target.value})} placeholder="Enter your email" className="w-full h-14 pl-11 pr-4 rounded-xl border border-slate-200 bg-white outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 text-sm font-medium transition-all" />
              </div>
            </div>
          </div>
          ...
          <div className="fixed bottom-0 left-0 right-0 bg-white border-t border-slate-100 z-50 p-4 pb-safe shadow-[0_-10px_40px_rgba(0,0,0,0.06)]">
            <div className="max-w-md mx-auto">
              <button disabled={loading} type="submit" className="w-full h-14 rounded-2xl bg-pink-500 hover:bg-pink-600 disabled:opacity-50 text-white font-extrabold text-lg flex items-center justify-center gap-2 shadow-lg shadow-pink-500/30 transition-all active:scale-[0.98]">
