      ...
      <div className="max-w-md mx-auto px-4 pt-6">
        <form onSubmit={handleSearch} className="relative">
          <input 
            ...
            className="w-full h-14 pl-5 pr-14 rounded-2xl border border-slate-200 bg-white outline-none focus:border-pink-500 focus:ring-2 focus:ring-pink-500/20 text-sm font-medium transition-all shadow-sm"
          />
          <button type="submit" disabled={loading} className="absolute right-2 top-2 w-10 h-10 bg-pink-500 hover:bg-pink-600 rounded-xl flex items-center justify-center text-white transition-colors">
          ...
          <div className="mt-6 animate-in slide-in-from-bottom-4 duration-500">
            ...
            <div className="mt-4 bg-white p-6 rounded-[24px] border border-slate-100 shadow-sm relative">
              ...
              <div className="flex gap-4 mb-8 relative z-10">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${currentStep >= 1 ? 'bg-pink-500 shadow-sm shadow-pink-500/30' : 'bg-slate-100 border-2 border-white'}`}>
              ...
              <div className="flex gap-4 mb-8 relative z-10">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${currentStep >= 2 ? 'bg-pink-500 shadow-sm shadow-pink-500/30' : 'bg-slate-100 border-2 border-white'}`}>
              ...
              <div className="flex gap-4 mb-8 relative z-10">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${currentStep >= 3 ? 'bg-pink-500 shadow-sm shadow-pink-500/30' : 'bg-slate-100 border-2 border-white'}`}>
              ...
              <div className="flex gap-4 relative z-10">
                <div className={`w-6 h-6 rounded-full flex items-center justify-center shrink-0 ${currentStep >= 4 ? 'bg-pink-500 shadow-sm shadow-pink-500/30' : 'bg-slate-100 border-2 border-white'}`}>
            ...
            <div className="mt-4 bg-purple-50 p-5 rounded-[24px] border border-purple-100 flex items-center gap-4">
              ...
              <button onClick={() => window.open(`https://wa.me/${business?.whatsapp_country_code || '91'}${business?.whatsapp_number}`, '_blank')} className="px-4 py-2.5 bg-pink-500 text-white font-extrabold text-xs rounded-xl shadow-sm whitespace-nowrap hover:bg-pink-600 transition-colors">
                Chat
              </button>
            </div>
          </div>
