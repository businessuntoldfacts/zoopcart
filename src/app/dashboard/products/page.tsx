    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6 pb-12">
        <div className="flex items-center justify-between">
          ...
          <Button onClick={handleAddProduct} variant="primary" className="rounded-xl px-6 bg-blue-500 hover:bg-blue-600 border-none shadow-md" disabled={submitting}>
        ...
        <form id="productForm" onSubmit={handleAddProduct} className="space-y-6">
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-slate-800 shadow-xl">
              <div className="flex flex-col items-center">
                <div className="w-40 h-40 rounded-2xl border-2 border-dashed border-slate-600 bg-black/50 overflow-hidden relative flex flex-col items-center justify-center text-center p-2 group hover:border-blue-500/50 transition-colors mb-6">
                  {formData.image ? (
                    <img src={formData.image} alt="Preview" className="w-full h-full object-cover absolute inset-0" />
                  ) : (
                    <>
                      <ImageIcon className="w-8 h-8 text-slate-500 mb-2 group-hover:text-blue-500" />
                ...
                {uploadingImage && <p className="text-xs text-blue-500 mt-2 font-bold animate-pulse">Optimizing image...</p>}
              </div>
            </div>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-slate-800 shadow-xl space-y-5">
              ...
              <div className="flex justify-between items-end mb-1.5">
                <label className="text-sm font-bold text-white">Name <span className="text-blue-500">*</span></label>
                <button onClick={handleAIGenerate} disabled={generatingAI} className="text-xs font-extrabold text-blue-400 bg-blue-500/10 px-2.5 py-1 rounded-lg flex items-center gap-1 hover:bg-blue-500/20 transition-colors">
              ...
              <div>
                <label className="text-sm font-bold text-white mb-1.5 block">Category <span className="text-blue-500">*</span></label>
                <select required value={formData.category} onChange={(e: any) => setFormData({...formData, category: e.target.value})} className="w-full bg-slate-50 border border-slate-200 text-slate-900 h-14 rounded-xl px-4 outline-none focus:border-blue-500 focus:ring-2 focus:ring-blue-500/20 appearance-none font-medium">
                ...
                <div className="grid grid-cols-2 gap-4">
                  <div>
                    <label className="text-sm font-bold text-white mb-1.5 block">Delivery <span className="text-blue-500">*</span></label>
                    <select required value={formData.delivery_type} onChange={(e: any) => setFormData({...formData, delivery_type: e.target.value})} className="w-full bg-black/50 border border-slate-700 text-white h-12 rounded-xl px-4 outline-none focus:border-blue-500 focus:ring-1 focus:ring-blue-500/50 appearance-none">
              ...
              <div>
                ...
                <textarea 
                  ...
                  className="w-full bg-black/50 border border-slate-700 text-white placeholder:text-slate-600 p-4 min-h-[160px] rounded-xl resize-y outline-none focus:border-blue-500 transition-colors" 
                />
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-slate-800 shadow-xl space-y-5">
              <div>
                <label className="text-sm font-bold text-white mb-1.5 block">Price <span className="text-blue-500">*</span></label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">₹</span>
              ...
              <div>
                <label className="text-sm font-bold text-white mb-1.5 block">MRP <span className="text-blue-500">*</span></label>
                <div className="relative">
                  <span className="absolute left-4 top-1/2 -translate-y-1/2 text-slate-500 font-bold">₹</span>
              ...
              <div>
                <label className="text-sm font-bold text-white mb-1.5 block">Stock <span className="text-blue-500">*</span></label>
          ...
          <div className="space-y-2">
            ...
            <div className="bg-[#1A1A1A] p-6 rounded-3xl border border-slate-800 shadow-xl space-y-6">
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setFormData({...formData, published: !formData.published})} className={`w-12 h-6 rounded-full transition-colors relative ${formData.published ? 'bg-blue-500' : 'bg-slate-700'}`}>
              ...
              <div className="flex items-center justify-between">
                ...
                <button type="button" onClick={() => setFormData({...formData, featured: !formData.featured})} className={`w-12 h-6 rounded-full transition-colors relative ${formData.featured ? 'bg-blue-500' : 'bg-slate-700'}`}>
  ...
  return (
    <div className="space-y-6 max-w-5xl mx-auto pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        ...
        <Button variant="primary" className="font-bold rounded-xl shadow-md bg-blue-500 hover:bg-blue-600 border-none" onClick={() => setShowAddForm(true)}>
      ...
      {products.length === 0 ? (
        <div className="bg-white rounded-3xl border border-slate-200 p-12 text-center shadow-sm">
          ...
          <Button variant="primary" className="font-bold rounded-xl bg-blue-500 hover:bg-blue-600 border-none" onClick={() => setShowAddForm(true)}>
            <Plus className="w-4 h-4 mr-2" /> Add Product
          </Button>
        </div>
      ) : (
            ...
            <div key={p.id} className="bg-white rounded-2xl p-4 border border-slate-200 shadow-sm flex flex-col sm:flex-row items-center gap-4 hover:shadow-md transition-shadow">
              ...
              <div className="flex sm:flex-col gap-2 w-full sm:w-auto mt-4 sm:mt-0 border-t sm:border-t-0 sm:border-l border-slate-100 pt-4 sm:pt-0 sm:pl-4">
                <button 
                  ...
                  className="flex-1 sm:flex-none flex items-center justify-center gap-2 text-xs font-bold text-slate-500 hover:text-blue-500 py-1 px-2 rounded-lg hover:bg-blue-50 transition-colors"
                >
